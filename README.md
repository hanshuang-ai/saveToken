...
```text
Bash 文本 -> 5,000 字符早期筛选 -> 严格结构化候选生成 -> 含标记的净收益检查
                                                   -> 同步提交原文 + FTS + 度量
                                                   -> 返回精简视图 + handle 标记
其他输入 / 无合格候选 / 提交失败 -> 原样放行
```

- **输入边界**:hook 只处理 Bash 返回的文本字符串,或对象中字符串类型的 `stdout` / `stderr`。不遍历任意字段,不处理任意二进制字段。`Read` 输出保持原样,避免预览后再次取回造成额外上下文。
- **早期筛选**:5,000 字符只是 Bash 结构化候选的字符门槛,不是 token 门槛,也不是强制压缩条件。超过门槛仍需通过严格识别和净收益检查。
...
- **诊断保留**:日志摘要保留所有识别出的信号行及其前后上下文和原始行范围;高密度诊断、信号过多的日志不摘要。快照摘要保留每个时间片的汇总和前几条高排名进程,并明确提示省略行可从原文取回。SGR 清理是独立且仅限上述范围的转换,不是改写诊断文本的许可。不对未知 build、grep、lint、JSON 或代码输出做泛化摘要。
...
| 工具 | 用途 |
|---|---|
| `tok_retrieve` | 指定已存 `handle`,用 `query` 检索或 `startLine` + `count` 按行取回;按行结果最多 160 行/12,000 字符,超长行可用 `offsetChars` 续读;只传 handle 可取全文,上限 60,000 字符 |
...
保留 codegraph 工具是为了已有持久化代码句柄;当前 v2 不压缩 `Read`,因此不会为新的 `Read` 自动生成代码句柄。
